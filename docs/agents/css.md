...
- **Roles, not palette.** Components use the `--color-*` role tokens (`--color-surface`, `--color-text-muted`, `--color-border`, ...). Palette tokens (`--teal-700`, `--sand-400`, ...) appear only inside `:root`, where roles are defined. Dark mode is the `prefers-color-scheme: dark` block right after `:root`: it redefines roles and nothing else. A new role needs a value in both, with text at WCAG AA contrast against the surfaces it sits on.
...
- Screenshots of every page at phone, tablet, and desktop widths, in light and dark, match before and after, except for the change you intended.
